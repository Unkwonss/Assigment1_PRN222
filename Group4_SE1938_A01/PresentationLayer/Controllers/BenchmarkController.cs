    ...
    public class BenchmarkController : Controller
    {
        ...
        public async Task<IActionResult> Dashboard()
        {
            ...
            ViewBag.TokenStats = stats.TokenStats.Select(x => new UserTokenStatsViewModel
            {
                ...
                FullName = x.FullName,
                Email = x.Email,
                TotalTokensIn = x.TotalTokensIn,
                TotalTokensOut = x.TotalTokensOut,
                MessageCount = x.MessageCount
            }).ToList();

            ViewBag.ModelComparison = stats.ModelComparison.Select(x => new ModelComparisonViewModel
            {
                ModelName = x.ModelName,
                AvgPrecision = x.AvgPrecision,
                AvgRecall = x.AvgRecall,
                AvgMRR = x.AvgMRR,
                AvgLatency = x.AvgLatency,
                TestCount = x.TestCount
            }).ToList();

            ViewBag.TopDocs = stats.TopDocs.Select(x => new TopDocumentViewModel
            {
                Title = x.Title,
                FileName = x.FileName,
            ...
            ViewBag.TokenOverTime = stats.TokenOverTime.Select(x => new TokenOverTimeViewModel
            {
                DateStr = x.DateStr,
            ...
            ViewBag.HourlyActivity = stats.HourlyActivity.Select(x => new HourlyActivityViewModel
            {
                Hour = x.Hour,
                Count = x.Count
            }).ToList();

            ViewBag.WordCloud = stats.WordFrequencies.Select(x => new WordFrequencyViewModel
            {
                Word = x.Word,
                Count = x.Count
            }).ToList();

            ViewBag.StrategyComparison = stats.StrategyComparison.Select(x => new StrategyComparisonViewModel
            {
            ...
            ViewBag.SubjectTokenStats = stats.SubjectTokenStats.Select(x => new SubjectTokenStatsViewModel
            {
            ...
            ViewBag.SubjectDocStats = stats.SubjectDocStats.Select(x => new SubjectDocStatsViewModel
            {
                SubjectCode = x.SubjectCode,
                SubjectName = x.SubjectName,
                DocumentCount = x.DocumentCount,
                IndexedCount = x.IndexedCount,
